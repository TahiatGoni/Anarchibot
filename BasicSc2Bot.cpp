...
void BasicSc2Bot::need_reinceforcements() {
	const sc2::ObservationInterface* observation = Observation();
	const sc2::GameInfo& game_info = Observation()->GetGameInfo();
...
void BasicSc2Bot::launch_attack() {
...
void BasicSc2Bot::finish_stragglers() {
...
bool BasicSc2Bot::TryUpgradeBarracks() {
	const sc2::ObservationInterface* observation = Observation();
...
bool BasicSc2Bot::TryUpgradeFactory() {
	const sc2::ObservationInterface* observation = Observation();
...
bool BasicSc2Bot::TryCollectGas() {
	...
	for (const auto& unit : units) {
		...
		if (target->assigned_harvesters < target->ideal_harvesters) {
			for (const auto& unit : units_SCV) {
				...
				if (unit->unit_type == sc2::UNIT_TYPEID::TERRAN_SCV && collectors.size() < target->ideal_harvesters - target->assigned_harvesters) {
